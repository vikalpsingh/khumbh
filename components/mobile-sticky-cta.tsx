...
export function MobileStickyCTA() {
  ...
  return <div className="fixed inset-x-0 bottom-0 z-50 border-t border-stone-200 bg-white/95 p-2 shadow-[0_-10px_30px_rgba(0,0,0,.08)] backdrop-blur md:hidden print:hidden"><div className="grid grid-cols-3 gap-2">
    <Link href={localizedHref("/plan-my-trip", locale)} className="flex min-h-12 flex-col items-center justify-center rounded-xl bg-saffron text-[11px] font-bold text-white"><MapPin className="mb-0.5 h-4 w-4" />{copy?.planTrip || "Plan Trip"}</Link>
    ...
    <a href={`https://wa.me/?text=${encodeURIComponent(`${copy?.siteName || "Ujjain Kumbh 2028"}: https://ujjain2028.in${locale === "en" ? "" : `/${locale}`}`)}`} target="_blank" rel="noreferrer" className="flex min-h-12 flex-col items-center justify-center rounded-xl bg-[#168f4d] text-[11px] font-bold text-white"><MessageCircle className="mb-0.5 h-4 w-4" />WhatsApp</a>
  </div></div>;
}
