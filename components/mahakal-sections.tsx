  ...
  return (
    <section className="brand-gradient temple-silhouette relative overflow-hidden text-white">
      ...
      <div className="relative mx-auto grid min-h-[580px] max-w-7xl items-center gap-10 px-4 py-20 sm:px-6 lg:grid-cols-[1fr_.6fr] lg:px-8">
        ...
        <div className="mx-auto hidden aspect-square w-full max-w-sm place-items-center rounded-full border border-gold/30 bg-black/10 lg:grid">
          <div className="grid h-48 w-48 place-items-center rounded-full border border-white/15 bg-white/5 text-center"><span className="font-serif text-7xl text-gold">ॐ</span></div>
        </div>
      </div>
    </section>
  );
...
export function BhasmaAartiTimeline({ steps }: { steps: { title: string; description: string }[] }) {
  return (
    <div className="overflow-hidden rounded-[2rem] border border-gold/40 bg-white shadow-soft">
      <div className="brand-gradient temple-silhouette p-7 text-white sm:p-10"><div className="flex items-center gap-3"><MoonStar className="h-7 w-7 text-gold" /><div><p className="text-xs font-extrabold uppercase tracking-[.2em] text-gold">Step-by-step</p><h3 className="mt-1 font-serif text-3xl text-white">Bhasma Aarti preparation</h3></div></div></div>
